...
source = "./modules/internetGW_module"
...
module "route_table_module" {
  ...
  for_each = var.aws_route_table_config
    ...
    internetGW_id = each.value.private == 0 ? module.internetGW_module[each.value.gateway_name].internetGW_id : module.Nat_GW_module[each.value.gateway_name].natGW_id 
